...
def modal_get_tools():
    ...
    try:
        logger.info("Starting modal_get_tools")
        
        # Try async approach first, fallback to simple approach
        try:
            tools = run_async_in_thread(handle_list_tools())
            tools_data = [tool.model_dump() for tool in tools]
        except Exception as async_error:
            logger.warning(f"Async approach failed: {async_error}, using fallback")
            tools_data = TOOLS_FALLBACK
...
def modal_call_tool(request: dict):
    ...
    try:
        logger.info(f"Starting modal_call_tool with request: {request}")
        ...
        # Try async approach first, fallback to direct implementation
        try:
            global analyzer
            if analyzer is None:
                analyzer = CloudStockAnalyzer()
            ...
            logger.warning(f"Async approach failed: {async_error}, using direct implementation")
...
def test_endpoint():
    ...
    try:
        ...
        return {
            ...
            "test_data": "AAPL stock test would work"
        }
    except Exception as e:
        return {
